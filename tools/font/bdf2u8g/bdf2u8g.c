...
unsigned char bdf_aa_bitmap_line[BDF_MAX_HEIGHT + 2 * BDF_AA_OFFSET][(20
		+ 2 * BDF_AA_OFFSET) * 8];
...
#else
	printf("bbx %d %d %d %d encoding %d\n", bdf_char_width, bdf_char_height,
			bdf_char_x, bdf_char_y, bdf_encoding);
	fflush(stdout);
#endif
}
...
void bdf_PutGlyph(void) {
	...
	if (bdf_state == BDF_STATE_ENCODING) {
		...
		if (bdf_font_format <= 1) {
			...
			len = (bdf_char_width + 2 * BDF_AA_OFFSET + 3) / 4
					* (bdf_char_height + 2 * BDF_AA_OFFSET);
		}
		if (len > 255) {
		...
		if (bdf_font_format == 0) {
			...
			if (bdf_glyph_data_len < 0 || bdf_glyph_data_len > 15) {
				fprintf(stderr,
						"Glyph with encoding %d does not fit for format 1 (data len = %d)\n",
						bdf_encoding, bdf_glyph_data_len);
				exit(1);
			}
			if (bdf_delta_x < 0 || bdf_delta_x > 15) {
				fprintf(stderr,
						"Glyph with encoding %d does not fit for format 1 (DWIDTH = %d)\n",
						bdf_encoding, bdf_delta_x);
				exit(1);
			}
			if (bdf_char_x < 0 || bdf_char_x > 15) {
				fprintf(stderr,
						"Glyph with encoding %d does not fit for format 1 (x-off = %d)\n",
						bdf_encoding, bdf_char_x);
				exit(1);
			}
			if (bdf_char_y < -2 || bdf_char_y > 13) {
				fprintf(stderr,
						"Glyph with encoding %d does not fit for format 1 (y-off = %d [%d..%d])\n",
						bdf_encoding, bdf_char_y, bdf_char_min_y,
						bdf_char_max_y);
				exit(1);
			}
			if (bdf_char_width < 0 || bdf_char_width > 15) {
				fprintf(stderr,
						"Glyph with encoding %d does not fit for format 1 (width = %d)\n",
						bdf_encoding, bdf_char_width);
				exit(1);
			}
			if (bdf_char_height < 0 || bdf_char_height > 15) {
				fprintf(stderr,
						"Glyph with encoding %d does not fit for format 1 (height = %d)\n",
			...
			if (((bdf_char_x << 4) | (bdf_char_y + 2)) == 255) {
				fprintf(stderr,
						"Glyph with encoding %d does not fit for format 1 (skip mark generated)\n",
						bdf_encoding);
				exit(1);
			}
		...
		sprintf(bdf_info + strlen(bdf_info),
				"/* encoding %d %c, bbx %d %d %d %d  asc %d dx %d*/\n",
				bdf_encoding,
				bdf_encoding > 32 && bdf_encoding <= 'z' ? bdf_encoding : ' ',
				bdf_char_width, bdf_char_height, bdf_char_x, bdf_char_y,
				bdf_char_ascent, bdf_delta_x);
...
int bdf_aa_matrix[9] = { 1, 3, 1, 3, 4, 3, 1, 3, 1 };
int bdf_aa_sum = 20;
			...
			if (bdf_GetXYVal(x - BDF_AA_OFFSET, y - BDF_AA_OFFSET) == 0) {
				...
				for (sy = -BDF_AA_OFFSET; sy <= BDF_AA_OFFSET; sy++) {
					for (sx = -BDF_AA_OFFSET; sx <= BDF_AA_OFFSET; sx++) {
						val = bdf_GetXYVal(x + sx - BDF_AA_OFFSET,
								y + sy - BDF_AA_OFFSET);
						val *= bdf_aa_matrix[(sy + BDF_AA_OFFSET)
								* (2 * BDF_AA_OFFSET + 1) + sx + BDF_AA_OFFSET];
						sum += val;
					}
				}
				if (sum <= 5) {
					...
					gray = (sum * (bdf_aa_gray_levels - 1) + (bdf_aa_sum / 2))
							/ bdf_aa_sum;
				}
...
void bdf_WriteC(const char *outname, const char *fontname) {
	...
	fprintf(out_fp,
			"  Calculated Max Values w=%2d h=%2d x=%2d y=%2d dx=%2d dy=%2d ascent=%2d len=%2d\n",
			bdf_char_max_width, bdf_char_max_height, bdf_char_max_x,
			bdf_char_max_y, bdf_delta_max_x, bdf_delta_max_y,
	...
	fprintf(out_fp,
			"  Calculated Min Values           x=%2d y=%2d dx=%2d dy=%2d\n",
	...
	fprintf(out_fp,
			"const u8g_fntpgm_uint8_t %s[%d] U8G_FONT_SECTION(\"%s\") = {\n",
...
int main(int argc, char **argv) {
	...
	if (argc < 4) {
		...
		printf(
				"%s [-l page] [-u page] [-s shift] [-S upper-shift] [-b begin] [-e end] [-f format] fontfile fontname outputfile\n",
				argv[0]);
		return 1;
	}
